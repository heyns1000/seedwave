...
// It handles mock brand data and acts as a local data source for development.
// It is intended to run locally via `node server.js` in a dedicated terminal
// when you are developing your frontend against a local data source.
...
// --- Middleware ---
app.use(cors()); // Enable CORS for all routes, allowing your frontend to access it locally
app.use(express.json()); // To parse JSON request bodies sent from the frontend

// --- Mock Data Definitions (Source of Truth for Local Backend) ---
// These are hardcoded for simplicity in this mock backend.
// In a real application, this data would be fetched from a persistent database (e.g., Firestore, MongoDB).
...
const ALL_BRANDS_DETAILS_BY_SECTOR = {
    ...
    "banking": [
        ...
        // Duplicate declaration removed. The original declaration remains intact.


// --- Mock Database for Brands Data (in-memory for local server) ---
// This will simulate your persistent brand data. It's initialized with static data.
let MOCK_BRANDS_DATABASE = {}; 

// Initialize MOCK_BRANDS_DATABASE with default static data
function initializeMockBrandsDatabase() {
    for (const sectorKey in ALL_BRANDS_DETAILS_BY_SECTOR) {
        const brandsInSector = ALL_BRANDS_DETAILS_BY_SECTOR[sectorKey];
        // Ensure brandsInSector is an array of objects, each with 'name' and 'subNodes'
    ...
    console.log("Mock Brands Database Initialized.");
}

// Execute database initialization when the server starts
initializeMockBrandsDatabase();


// --- Mock Database for PayPal Plan IDs (in-memory for local server) ---
// In a real app, this would be fetched from PayPal API and cached/stored persistently.
const MOCK_PAYPAL_PLAN_DATABASE = {};

// Function to generate and store mock PayPal Plan IDs
// This populates MOCK_PAYPAL_PLAN_DATABASE for the `/api/paypal/plans` endpoint on this server.
function generateAndStoreMockPlanIDs() {
    const annualDiscountFactor = 0.85; // 15% discount for annual plans (1 - 0.15)

    for (const sectorKey in FAA_ZONE_INDEX_SUMMARY_DATA) {
        if (sectorKey === 'admin-panel') continue; 

        const sectorDisplayName = sectorList[sectorKey].replace(/<.*?>/g, '').trim(); 
        ...
        for (const tierName in SECTOR_TIER_PRICING) {
            const tierMultiplier = SECTOR_TIER_PRICING[tierName].multiplier;
            
            const monthlyAmount = (baseMonthlyFee * tierMultiplier).toFixed(2);
            const annualAmount = (baseMonthlyFee * tierMultiplier * 12 * annualDiscountFactor).toFixed(2);

            // Constructing plan keys consistent with frontend naming:
            // "Sector Name Tier Package_BILLING_CYCLE"
            const productBaseName = `${sectorDisplayName} ${tierName} Package`;
            
            // Generate unique mock PayPal Plan IDs (these are just examples for local mock)
            const monthlyPlanId = `P-MOCK-${sectorKey.toUpperCase()}-${tierName.toUpperCase()}-MONTHLY-${Math.floor(Math.random() * 99999).toString().padStart(5, '0')}`;
            const annualPlanId = `P-MOCK-${sectorKey.toUpperCase()}-${tierName.toUpperCase()}-ANNUAL-${Math.floor(Math.random() * 99999).toString().padStart(5, '0')}`;

            MOCK_PAYPAL_PLAN_DATABASE[`${productBaseName}_MONTHLY`] = monthlyPlanId;
            MOCK_PAYPAL_PLAN_DATABASE[`${productBaseName}_ANNUAL`] = annualPlanId;
        }
    }
    // Override Agriculture Starter Monthly with the specific one you provided earlier for consistency
    // Note: This ID 'P-07F980334R518562XNBHLNJY' is actually a PayPal Product ID from your frontend,
    // but here it's used as a mock Plan ID. This is acceptable for a local mock server.
    MOCK_PAYPAL_PLAN_DATABASE["Agriculture & Biotech Starter Package_MONTHLY"] = 'P-07F980334R518562XNBHLNJY';
    console.log("Mock PayPal Plan IDs generated and stored in MOCK_PAYPAL_PLAN_DATABASE.");
}

// Execute plan ID generation on server start
generateAndStoreMockPlanIDs();


// --- API Endpoints for Local Backend ---

// Endpoint to fetch all brand data by sector
app.get('/api/sectors', (req, res) => {
    // Return the current state of the mock brands database
...
app.post('/api/sectors', (req, res) => {
    ...
    if (sector && brand && subnodes && Array.isArray(subnodes)) {
        ...
        res.status(400).json({ success: false, message: 'Invalid request body. Requires sector (string), brand (string), and subnodes (array).' });
    }
});


// Note: The /api/paypal/* endpoints are now handled by the api/index.js serverless function on Vercel.
// This local server DOES NOT handle those PayPal API calls directly anymore.
// The `MOCK_PAYPAL_PLAN_DATABASE` generated here is purely for mocking purposes
// if this server were to ever provide mock PayPal plan data for an endpoint it serves.
// For *this* specific project structure, your frontend fetches PayPal plans and
// creates plans directly from the Vercel serverless function, NOT from this local Express server.
...
app.listen(port, () => {
    ...
    console.log(`Ensure your frontend's BACKEND_LOCAL_SERVER_URL in admin-panel.html is set to http://localhost:${port}`);
});
