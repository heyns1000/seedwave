...
// Note: @paypal/checkout-server-sdk is not directly used for API calls in this version
// as node-fetch is used for more direct control of PayPal API.
...
// Provide fallback values from your Zoho credentials and product ID for local development if not set in .env.local
...
const PAYPAL_WEBHOOK_ID = process.env.PAYPAL_LIVE_WEBHOOK_ID || 'YOUR_SANDBOX_WEBHOOK_ID_GOES_HERE'; // <--- YOU MUST REPLACE THIS if using webhooks
const PAYPAL_WEBHOOK_SECRET = process.env.PAYPAL_LIVE_WEBHOOK_SECRET || 'YOUR_SANDBOX_WEBHOOK_SIGNING_SECRET_GOES_HERE'; // <--- YOU MUST REPLACE THIS if using webhooks

// Define your single product ID to which all plans will be linked
// This should match the Product ID you create in your PayPal Developer Dashboard (Sandbox or Live)
const YOUR_PAYPAL_PRODUCT_ID_FOR_PLANS = process.env.PAYPAL_LIVE_PRODUCT_ID || 'P-07F980334R518562XNBHLNJY'; // Your default Product ID
...
const FAA_ZONE_INDEX_SUMMARY_DATA = {
    ...
    "admin-panel": "⚙️ Admin Panel" // Not a real sector, but kept for list completeness
};
        ...
        switch (action) {
            case 'get-plans': {
                ...
                plans.forEach(plan => {
                    ...
                    if (cleanName && billingCycle) {
                        ...
                        let frontendKey = cleanName.replace(/\s/g, '_');
                        // Apply specific replacements if PayPal names don't directly match frontend for _ separator
                        // For example, "Agriculture & Biotech" might need to become "Agriculture___Biotech"
                        // This assumes your PayPal plan name format is e.g. "Agriculture & Biotech Starter Package (Monthly)"
            ...
            case 'create-plans-for-sector': {
                ...
                for (const planData of planDetailsArray) {
                    ...
                    try {
                        const planBody = {
                            product_id: YOUR_PAYPAL_PRODUCT_ID_FOR_PLANS, // Use the configured product ID
                            name: planData.name,
            ...
            case 'create-subscription': {
                ...
                const subscriptionBody = {
                    ...
                    application_context: {
                        // IMPORTANT: Change these return/cancel URLs to your actual frontend URLs
                        return_url: process.env.FRONTEND_URL ? `${process.env.FRONTEND_URL}/subscription-success` : 'http://localhost:3000/subscription-success', 
                        cancel_url: process.env.FRONTEND_URL ? `${process.env.FRONTEND_URL}/subscription-cancel` : 'http://localhost:3000/subscription-cancel',
                        brand_name: 'FAA.ZONE™',
                        locale: 'en-ZA',
                        shipping_preference: 'NO_SHIPPING_ADDRESS' // Adjust as needed
                    }
                };
