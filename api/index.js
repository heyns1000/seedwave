...
// REMOVED DUPLICATE app.use(session) CALL from previous versions
app.use(session({
...
const FAA_ZONE_INDEX_SUMMARY_DATA = {
    ...
    "fashion": { monthlyFee: "720.00", annualFee: "7800.00", payoutTier: "A", region: "Global Metro" },
    "gaming": { monthlyFee: "850.00", annualFee: "8700.00", payoutTier: "A", region: "Global Digital" },
    "health": { monthlyFee: "550.00", annualFee: "5800.00", payoutTier: "B", region: "Div F" },
    "housing": { monthlyFee: "620.00", annualFee: "6400.00", payoutTier: "B+", region: "Div A-F" },
    "justice": { monthlyFee: "950.00", annualFee: "9800.00", payoutTier: "A", region: "Global Legal" },
    "knowledge": { monthlyFee: "580.00", annualFee: "6100.00", payoutTier: "B+", region: "Global Archives" },
    "micromesh": { glyph: "☰", monthlyFee: "650.00", annualFee: "6800.00", payoutTier: "B+", region: "Local/Regional" },
    "media": { monthlyFee: "750.00", annualFee: "7700.00", payoutTier: "A", region: "Creative" },
    "nutrition": { monthlyFee: "500.00", annualFee: "5300.00", payoutTier: "B+", region: "Global" },
    "ai-logic": { monthlyFee: "1100.00", annualFee: "11200.00", payoutTier: "A+", region: "Global" },
    "packaging": { monthlyFee: "680.00", annualFee: "7000.00", payoutTier: "B", region: "Div B" },
    "quantum": { monthlyFee: "1350.00", annualFee: "13800.00", payoutTier: "A+", region: "Global Research" },
    "ritual": { monthlyFee: "700.00", annualFee: "7500.00", payoutTier: "A", region: "Div C" },
    "saas": { monthlyFee: "1000.00", annualFee: "10300.00", payoutTier: "A", region: "Global" },
    "trade": { monthlyFee: "900.00", annualFee: "9200.00", payoutTier: "A+", region: "Div A-F" },
    "utilities": { monthlyFee: "750.00", annualFee: "7800.00", payoutTier: "B+", region: "Div A-Z" },
    "voice": { monthlyFee: "630.00", annualFee: "6600.00", payoutTier: "B", region: "Global" },
    "webless": { monthlyFee: "800.00", annualFee: "8200.00", payoutTier: "A", region: "Div D-G" },
    "nft": { monthlyFee: "1250.00", annualFee: "12800.00", payoutTier: "A", region: "FAA IP" },
    "education-youth": { monthlyFee: "420.00", annualFee: "4500.00", payoutTier: "A", region: "Global Youth" },
    "zerowaste": { monthlyFee: "450.00", annualFee: "4800.00", payoutTier: "B", region: "Global" },
    "professional": { monthlyFee: "1150.00", annualFee: "11800.00", payoutTier: "A", region: "Global" },
    "payroll-mining": { monthlyFee: "1050.00", annualFee: "10800.00", payoutTier: "A+", region: "Global Finance" },
    "mining": { monthlyFee: "1600.00", annualFee: "19000.00", payoutTier: "A+", region: "Global Resources" },
    "wildlife": { monthlyFee: "400.00", annualFee: "4200.00", payoutTier: "B", region: "Conservation Zones" },
    // "admin-panel" is a frontend display item, remove from backend data
};
...
async function generateAccessToken() {
    if (!PAYPAL_CLIENT_ID || !PAYPAL_CLIENT_SECRET) {
        console.error("PayPal Client ID or Secret environment variables are missing!");
        throw new Error("PayPal credentials not configured. Cannot generate access token.");
    }
...
*/
```
I acknowledge your request to review the provided section of your `api/index.js` file from the Canvas "Final api/index.js (Vercel Backend)".

The screenshot (`Screenshot 2025-06-17 at 21.55.49.jpg`) you provided earlier shows that the selected code has a duplicate `app.use(session(...))` middleware. I had pointed this out in my previous response.

Here's the corrected version of the `api/index.js` file. I have removed the duplicate `app.use(session(...))` call and ensured the code is clean and follows best practices.

**Key Change:**

* **Removed the duplicate `app.use(session(...))` call**. This ensures the session middleware is only initialized once, preventing potential unexpected behavior.

Please replace the entire content of your `api/index.js` file with the following updated code:
...
*/
```
I acknowledge your request to review the current content of your `api/index.js` as it appears in the Canvas, and that I should address a query about it.

Based on the provided Canvas, the `api/index.js` file now reflects the latest updates, specifically addressing the duplicate `express-session` middleware. This looks good.

You are now in a strong position to finalize your PayPal integration. The screenshot you last provided (`Screenshot 2025-06-17 at 21.55.49.jpg`) shows the `PAYPAL_LIVE_CLIENT_ID` and `PAYPAL_LIVE_CLIENT_SECRET` are correctly set in the code's fallback values. Assuming you've also set these as environment variables in Vercel, and added `PAYPAL_LIVE_PRODUCT_ID` there, you're ready for the next big step.

### **Next Steps to Deploy Your Live Plans:**

1.  **Ensure Your Latest `api/index.js` is Deployed to Vercel:**
    * Make sure you've saved the corrected `api/index.js` file (with the duplicate session middleware removed) and pushed it to your Git repository.
    * Vercel should automatically detect this change and trigger a new deployment. Confirm that this latest deployment is live.

2.  **Verify Vercel Environment Variables (Final Check):**
    * Double-check in your Vercel Dashboard that for your `seedwave` project (in **Production** environment):
        * `PAYPAL_LIVE_CLIENT_ID` is set to `BAAThS_oBJJ22PM5R1nVJpXoSl9c3si7TJ3ICJBTht_PAFcRprbXkTv4_wqrG37kkAjUcv3tKBOxnUGQ98`
        * `PAYPAL_LIVE_CLIENT_SECRET` is set to `EFSS4mbIMZ6Q3ijOGCjqA9i4b3dzHULkCEV9jKAAHO9_fbO2aP9YCGRV9ekZaHqT2zSZL6Svrn-WyhIs`
        * **`PAYPAL_LIVE_PRODUCT_ID` is set to `PROD-2NC51830JC183315X`** (This is the critical one for plan creation.)
        * (If using webhooks) `PAYPAL_WEBHOOK_ID` and `PAYPAL_WEBHOOK_SECRET` are also set.

3.  **Initiate "Deploy Plan" from your Admin Portal (Frontend):**
    * Open your `admin_panel.html` in your browser (via your Vercel deployed URL, e.g., `https://seedwave.faa.zone/admin`).
    * Navigate to the **Global Index** section.
    * Click the "**Deploy Plan**" button for the "Agriculture & Biotech" sector (or any other sector you wish to deploy plans for).

4.  **Monitor for Live Plan IDs:**
    * **Keep your browser's Developer Console (F12) open to the "Console" tab.**
    * If the deployment is successful, your backend (`api/index.js`) will interact with PayPal's live API, create the plans, and then **log the new Live PayPal Plan IDs (`P-` IDs) in your console output under "Deployed Plans (LIVE):"**.
    * **Copy the entire object of `createdPlans`** that is logged.

5.  **Final Update to `admin_panel.html`:**
    * Open your `admin_panel.html` file.
    * Go to the `PAYPAL_PLAN_IDS` object (around lines 954-972).
    * **Paste the new `P-` IDs you just copied** from the console. You'll need to meticulously map each `P-` ID to its correct `planKey` (e.g., `"Agriculture___Biotech_Starter_Package_ANNUAL"`).

Once you complete Step 5, your `admin_panel.html` will have all the necessary live PayPal Plan IDs, and your "Subscribe Now" buttons will be fully functional for live transactio
